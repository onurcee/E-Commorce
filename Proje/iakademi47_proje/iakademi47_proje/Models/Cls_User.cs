...
namespace iakademi47_proje.Models
{
	public class Cls_User
	{
        ...
        public static bool SendSms(string OderGroupGUID)
        {
            try
            {
                using (iakademi47Context context = new iakademi47Context())
                {
                    ...
                    ss += "<mainbody>";
                    ss += "<header>";
                    ss += "<company dil= 'TR'>üyelikte size verilen şirket ismi yazılacak</company>";
                    ss += "<useercode>üye olunca verieln usercode</usercode>";
                    ...
                    int UserID = context.Orders.FirstOrDefault(o => o.OderGroupGUID == OderGroupGUID).UserID;

                    User user = context.Users.FirstOrDefault(u => u.UserID == UserID);

                    string icerik = "Sayın "+user.NameSurname+", "+DateTime.Now+" tarihinde "+OderGroupGUID+" nolu siparişiniz alınmıştır.";
                    ss += "<mp> <msg> <!CDATA["+icerik+ "]> </msg> <no>"+user.Telephone+" </no> </mp>";
                    ss += "</body>";
                    ss += "</mainbody>";
                    ...
                    if (result != "-1")
                    {
                        //sms gitti,Order tablosuna sms kolonuna true bas
                    }
                    else
                    {
                        //sms gitmedi, Orders tablosuna sms false bas
        ...
        public static string XmlPost(string url, string ss)
        {
            try
            {
                ...
                HttpWebRequest request = WebRequest.Create(url) as HttpWebRequest;// casting = tip dönüştürme ya da (HttpWebRequest)WebRequest.Create(url)
                request.Method = "POST";
        ...
        public static void SendEmail(string OderGroupGUID)
        {
            using (iakademi47Context context = new iakademi47Context())
            {
                Order order = context.Orders.FirstOrDefault(o => o.OderGroupGUID == OderGroupGUID);
                ...
                content = "Sayın " + user.NameSurname + "," + DateTime.Now + " tarihinde " + OderGroupGUID + " nolu siparişiniz alınmıştır.";
