    ...
    public class Cls_Order
    {
        ...
        public void DeleteFromMyCart(string scid)
        {
            ...
            for (int i = 0; i < MyCartArray.Length; i++)
            {
                ...
                if (count == 1)
                {
                    //yeni sepet içine silinmeyen ürünleri koyalım
                    //yeni sepete ilk ürün koyuyorum
                ...
                {
					//count 1 üründen büyük, yani sepette en az bir ürün var
					if (MyCartArrayLoop[0] != scid)
					{
						NewMyCart += "&"+ MyCartArrayLoop[0] + "=" + Convert.ToInt32(MyCartArrayLoop[1]);
						count++;
        ...
        public List<Cls_Order> SelectMyCart()
        {
            ...
            if (MyCartArray[0] != "")
            {
                for (int i = 0; i < MyCartArray.Length; i++)
                {
                    ...
                    pr.ProductID = product.ProductID;//DB'den aldım property ye gönderdim
        ...
        {
            string OderGroupGUID = DateTime.Now.ToString().Replace(":", "").Replace(".", "").Replace(" ", "").Replace(",", "");
            ...
            foreach (var item in orders)
            {
                ...
                order.OderGroupGUID = OderGroupGUID;
